...
from sentence_classifier import get_results
...
def majority_voting(*prediction_sets):
    ...
    for row in stacked_predictions.T:  # Iterate over each set of predictions for a sample
        ...
        if len(row_counts) == 3:  # If all predictions are different
...
def main():
    ...
    training_data = load_data('OADS_Train3.json')  # Training input features
    testing_data = load_data('OADS_Test3.json')    # Testing input features
    ...
    classifier2_predictions = get_results()
    true_labels, predicted_labels = instructor.run()
    ...
    mapped_labels = predictions_df['label'].map(label_mapping).tolist()
    print("Mapped labels:", mapped_labels)

    # Combine predictions from different classifiers (including mapped_labels)
    final_predictions = majority_voting(predicted_labels, classifier2_predictions, predictions_df['predicted_label'])
    
    # Print classification reports
    print("Classifier 1 Classification Report:")
    print(classification_report(true_labels, predicted_labels))
    print("============================================================")
    print("Classifier 2 Classification Report:")
    print(classification_report(true_labels, classifier2_predictions))
    print("============================================================")
    print("Classifier 3 Classification Report:")
    print(classification_report(true_labels, predictions_df["predicted_label"]))
    print("============================================================")
    print("Final Predictions Classification Report:")
    print("Final Predictions Classification Report:")
